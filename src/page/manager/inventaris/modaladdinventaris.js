...
const ModalAddInventaris = ({ showModal, setShowModal, setInventarisList, token, onAddSuccess }) => {
  ...
  const handleSave = async () => {
    if (namaBarang.trim() === '' || tglBeli.trim() === '' || harga.trim() === '' || ket.trim() === '') {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/inventaris/add`, // Menggunakan URL dari .env
        { nama_barang: namaBarang, tgl_beli: tglBeli, harga: harga, ket: ket },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.status === 201) {
        onAddSuccess(response.data.data); // Callback untuk menambahkan inventaris baru
        setShowModal(false);
        toast.success('Inventaris berhasil ditambahkan!'); // Notifikasi sukses
      }
    } catch (err) {
      console.error('Error saat menyimpan inventaris:', err);
      toast.error('Gagal menambahkan inventaris'); // Notifikasi gagal
    }
  };
          ...
          <div className="modal-container">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Tambah Inventaris</h5>
                <button
                  type="button"
                  className="close"
...
export default ModalAddInventaris;